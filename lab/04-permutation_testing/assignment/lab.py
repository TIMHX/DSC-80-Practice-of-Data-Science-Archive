...
def total_seller(df):
    ...
    True
    """
    return pd.pivot_table(df, values='Total', index=['Name'], aggfunc=np.sum)
    ...
...
def product_name(df):
    ...
    0
    """
    return pd.pivot_table(df, index=['Product'], columns=['Name'], aggfunc=np.sum)
    ...
...
def count_product(df):
    ...
    70
    """
    return pd.pivot_table(df, index=['Product', 'Name'], columns=['Date'], aggfunc=np.count_nonzero, fill_value=0)
    ...
...
def diff_of_means(data, col='orange'):
    ...
    """
    ski_c = data.copy()
    return ski_c.groupby('Factory')[col].mean().diff().abs().to_list()[1]
    ...
...
def pval_orange(data, col='orange'):
    ...
    return np.count_nonzero(differences >= observed_diff) / n
    ...
...
def ordered_colors():
    ...
    True
    """
    return [('purple', 0.974), ('green', 0.4687), ('red', 0.247), ('orange', 0.044), ('yellow', 0.0003)]
    ...
